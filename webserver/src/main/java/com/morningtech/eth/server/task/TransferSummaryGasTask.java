 ...
 * @date 2018/1/8 18:11
 */
public class TransferSummaryGasTask implements Runnable {

    public final static Logger logger= LoggerFactory.getLogger(TransferSummaryGasTask.class);

    private TransferSummaryCoinManage transferSummaryCoinManage;

    private EthWalletManage ethWalletManage;

    private List<WalletUser> walletUserList;
    ...
    private Map<String, BigInteger>  toAccountEth_unitwei=Maps.newConcurrentMap();//缺少的矿工费

    public TransferSummaryGasTask(TransferSummaryCoinManage transferSummaryCoinManage, List<WalletUser> walletUserList) {
    ...
    public void run() {
        ...
        try {
             ...
             for (WalletUser walletUser:
                ...
                if (coinname!=null && !"".equals(coinname)) {
                    if(walletUser.getTranStatus().indexOf("1")!=-1){
                        toUserEthSet.add(accountId);//已经转过GAS到用户了,放入禁止打gas列表
                    }
                    ...
                    if(coinname.equals(EnumContractCoinname.ETH.getCoinname())){//如果是eth，则钱包账户所需的eth需要加上归集的数量
                        maxGas= maxGas.add(walletUser.getValueWei());
                    }
            ...
            for(String accountId : accountIdSet){
                BigInteger totalGas_unitwei=account_ether_unitwei.get(accountId);//归集账户所需的eth数量(eth归集数量+总矿工费)
               /**
                 * 查询用户账号eth数量是否满足矿工费用
                 */
                BigInteger eth_wei=ethWalletManage.getBlanceEth_UnitWei(accountId);//钱包eth数量
                //账户已有的eth数量少于矿工费用，需要主账户转移缺少的eth以支持转账
                if(eth_wei.compareTo(totalGas_unitwei)==-1){
                    toUserEthSet.remove(accountId);
             ...
             * ===================================================
             */
            int queEthAccountSize = toAccountEth_unitwei.size();
            if(queEthAccountSize>0) {
                totalEth_unitwei = totalEth_unitwei.add(BigInteger.valueOf(queEthAccountSize).multiply(ethGasWei));
            }
            //邮件内容
                                    ...
                                    public void toAddressGas() {
                                        try {
                                            ...
                                            ethContract.l_transfer(toAddressId, value).observable().doOnError(err -> {
                                                ...
                                                toUserEthSet.remove(toAddressId);//下一次检测重新打入
                                            }).subscribe(tx -> {
